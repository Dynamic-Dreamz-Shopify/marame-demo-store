                                ...
                                if(result.processedLineItems.includes(parseInt(lineItem.id.replace("gid://shopify/LineItem/",""))) != true){
                                    ...
                                    orderListHTML += `<div class="return-order-lineitem${result.processedLineItems.includes(parseInt(lineItem.id.replace("gid://shopify/LineItem/",""))) == true ? " hidden" : ""}" 
                                        ...
                                        <div class="return-order-lineitem-info">
                                            ...
                                            <div class="lineitem-info-price"><span>Purchase Price</span><span>${Shopify.formatMoney(totalToDisplay*100,document.body.getAttribute(`shop_moneyformat`)).replace(".00","").replace(",00","")}</span></div>
                                        </div>
                                    ...
                                    //                 <span>${Shopify.formatMoney(totalToDisplay*100, document.body.getAttribute("shop_moneyformat")).replace(".00","").replace(",00","")}</span>
                                    //             </div>
                                    //         </div>
...
function selectedProductsHTML(){
    ...
    document.querySelectorAll(`.return-order-lineitem.selected`).forEach((item,index) => {
        ...
        returnOptionGrid += `<div class="return-option-grid-item${index == 0 ? " active" : ""}" data-id="${item.dataset.id}" data-vid="${item.dataset.id}">
...
function exchangeORreturn_ProductsHTML(){
        ...
        selectExchangeArr.forEach(select => {
            select.addEventListener("change",function(e){
                ...
                if(option){
                    ...
                    const new_size_id = option.getAttribute('value');
                    ...
                    select.closest('.return-option-grid-item').querySelector('.return-option-card[data-option="exchange"].active').closest(".return-option-grid-item").setAttribute("data-vid",new_size_id);
                }
    ...
    //     document.querySelector(`[data-return-table-return-confirmation-total]`).innerHTML = Shopify.formatMoney((priceTotalReturnONLY*100),document.body.getAttribute(`shop_moneyformat`)).replace(".00","").replace(",00","") + "*";
    //     document.querySelector(`[data-return-table-return-confirmation-returntotal]`).innerHTML = Shopify.formatMoney((Math.abs(priceTotalReturnONLY - 5)*100),document.body.getAttribute(`shop_moneyformat`)).replace(".00","").replace(",00","") + "*";
    ...
    console.log("dataIdsArrayReturnStoreCradit:", dataIdsArrayReturnStoreCradit);
        ...
        dataIdsArrayExchange.forEach(id => {
            ...
            const exchangeCard = document.querySelector(
                `.return-option-grid-item[data-vid="${id}"] .return-option-card[data-option="exchange"]`
            );
...
if (
   ...
   document.querySelectorAll("[data-return-table-return-confirmationall] .return-table-reason-tr").forEach(element => {
    ...
    if(priceTotalReturn > 0 )
    {
        ...
        document.querySelector(`[data-return-table-return-confirmation-total]`).innerHTML = Shopify.formatMoney((priceTotalReturn * 100), document.body.getAttribute(`shop_moneyformat`)).replace(".00", "").replace(",00", "") + "*";
    }
    else
    {
...
// console.log("Final lineItems:", lineItems);
  ...
  let uniqueTypes = [...new Set(lineItems.map(item => item.return_type))];
    ...
    const selected_card =  document.querySelector(".return-print-card.return-option-selected").getAttribute("data-print"); 
     const selected_location =  document.querySelector(".return-from-location-item.active").dataset.returnFromLocationBtn;
     var is_location = false;
     var is_card = null ;

     console.log("selected_location:--",selected_location);
     if(selected_location == "GREAT BRITAIN")
     {
        is_location = true;
        is_card = selected_card ;
     }
